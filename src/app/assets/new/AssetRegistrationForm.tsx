...
function validateForm(form: FormState) {
  ...
  if (form.photoUrl.trim() && !/^https?:\/\/\S+$/i.test(form.photoUrl.trim())) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (Object.keys(errors).length > 0 || !hasCategories) {
      if (!hasCategories) {
        setMessage({
          kind: "error",
          text: "No active asset categories are available yet. Ask the Organization Setup owner to seed or create categories.",
        });
      }
      return;
    }
  ...
  return (
    ...
    >
      {!hasCategories ? (
        ...
        >
          No active asset categories are available. Asset Registration is ready,
          but it needs categories from the Organization Setup module before an
          asset can be saved.
        </div>
      ) : null}
      ...
      <form onSubmit={handleSubmit}>
        ...
        >
          ...
          <label>
            ...
            />
            {fieldErrors.photoUrl ? <FieldError>{fieldErrors.photoUrl}</FieldError> : null}
          </label>
        </div>
